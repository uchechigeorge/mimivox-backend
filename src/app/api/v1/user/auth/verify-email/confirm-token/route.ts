...
import { verifyEmailConfirmTokenValidator } from "@/lib/validators/user/auth.validator";
import { NextResponse } from "next/server";

export const POST = userHandler(
  async (req: Request, ctx, authItems) => {
    const body = await req.json();
    const dto = verifyEmailConfirmTokenValidator.parse(body);

    const result = await authService.verifyEmail.confirmToken(dto, authItems);
